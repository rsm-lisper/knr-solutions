...
int main (int argc, char *argv[])
{
  ...
  int dictionary = 0; /* 1 if dictionary order sort */
  int c;
  ...
  if ((numeric && fold) || (numeric && dictionary)) {
    printf("Option -f and -d can't be combined with -n\n");
    return 3;
  }
  if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {
    qsort((void**) lineptr, 0, nlines-1,
          ...
          (numeric && reverse ? num_rev_cmp :
           ...
           ( /* !numeric... */
             reverse && fold && dictionary ? str_rev_ic_dict_cmp :
             reverse && fold && !dictionary ? str_rev_ic_cmp :
             reverse && !fold && dictionary ? str_rev_dict_cmp :
             reverse && !fold && !dictionary ? str_rev_cmp :
             !reverse && fold && dictionary ? str_ic_dict_cmp :
             !reverse && fold && ! dictionary ? str_ic_cmp :
             !reverse && !fold && dictionary ? str_dict_cmp :
             str_cmp))); /* !reverse && !ignore_case && !dictionary */
